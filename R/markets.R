...
markets <- function(pair, params = NULL, exchange = "kraken", route = "ohlc", datetime = TRUE) {
  ...
  if(route != "ohlc") {
    ...
    stop("markets() only works with route 'ohlc'. For other routes use get_trades, get_summary, get_current_price,
         get_orderbook, get_orderbook_liquidity")
  ...
  data <- get_markets(route = route, pair = pair, exchange = exchange, params = params)

  df.data <- data.frame(data[[1]])
